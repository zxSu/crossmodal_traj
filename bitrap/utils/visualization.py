...
class Visualizer():
    ...
    def initialize_v2(self, img_path=None, img_shape_new=None):
        if self.mode == 'image':
            self.img = np.array(Image.open(img_path))
            #
            if img_shape_new is not None:
                self.img = cv2.resize(self.img, (img_shape_new[0], img_shape_new[1]), interpolation=cv2.INTER_NEAREST)
            #
            self.H, self.W, self.CH = self.img.shape
        elif self.mode == 'plot':
            self.fig, self.ax = plt.subplots()
    ...
    def visualize(self, 
                  ...
                  crossing_flag=None):
        if viz_type == 'bbox':
            self.viz_bbox_trajectories(inputs, normalized=normalized, label=label, bbox_type=bbox_type, color=color, thickness=thickness, viz_time_step=viz_time_step, crossing_flag=crossing_flag)
        elif viz_type == 'point':
        ...
        elif viz_type == 'bbox_only':
            self.viz_bbox_only(inputs, normalized=normalized, label=label, bbox_type=bbox_type, color=color, viz_time_step=viz_time_step)
    ...
    def viz_point_trajectories(self, points, color=(255,0,0), label=None, thickness=4, radius=5):
        ...
        if self.mode == 'image':
            ...
            for k in range(K):
                ...
                for t in range(T):
                    cv2.circle(self.img, tuple(points[t, k, :]), color=color, radius=radius, thickness=2)    # original: thickness=-1
        elif self.mode == 'plot':
    ...
    def draw_single_bbox(self, bbox, color=None, thickness=2):
        '''
        img: a numpy array
    ...
    def viz_bbox_trajectories(self, bboxes, normalized=False, label='', bbox_type='x1y1x2y2', color=None, thickness=4, radius=5, viz_time_step=None, 
                              crossing_flag=None):
        '''
        bboxes: (T,4) or (T, K, 4)
        '''
        ...
        self.viz_point_trajectories(center_points, color=color, thickness=4, radius=radius)
        ...
        for t in range(T):
            for k in range(K):
                self.draw_single_bbox(bboxes[t, k, :], color=color, thickness=thickness)    # emmmm, it draws all sampled predicted trajectories.
        
        # draw crossing text
        if crossing_flag is not None:
            str_crossing = None
            if crossing_flag==1:
                str_crossing = 'crossing'
            elif crossing_flag==0:
                str_crossing = 'not_crossing'
            else:
                str_crossing = 'unknown'
            str_crossing = str_crossing + ' ' + label
            text_loc = (bboxes[0, 0, 0], bboxes[0, 0, 1])
            cv2.putText(self.img, str_crossing, text_loc, cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
    
    
    
    def viz_bbox_only(self, bboxes, normalized=False, label='', bbox_type='x1y1x2y2', color=None, viz_time_step=None):
        '''
        bboxes: (T,4) or (T, K, 4)
        '''
        if len(bboxes.shape) == 2:
        ...
        for t in range(T):
            for k in range(K):
                self.draw_single_bbox(bboxes[t, k, :], color=color)    # emmmm, it draws all sampled predicted trajectories.
    
    
        
        
    def viz_goal_map(self, img, goal_map):
        '''
        img:
    ...
    def viz_crossing_flag(self, crossing_pred, crossing_gt, bboxLast_pred, bboxLast_gt, color_pred, color_gt):
        str_pred = None
        if crossing_pred==1:
            str_pred = 'crossing_pred'
        else:
            str_pred = 'not_crossing_pred'
        cv2.putText(self.img, str_pred, bboxLast_pred, cv2.FONT_HERSHEY_SIMPLEX, 2, color_pred, 2)
        #
        str_gt = None
        if crossing_gt==1:
            str_pred = 'crossing_gt'
        else:
            str_pred = 'not_crossing_gt'
        cv2.putText(self.img, str_gt, bboxLast_gt, cv2.FONT_HERSHEY_SIMPLEX, 2, color_gt, 2)
