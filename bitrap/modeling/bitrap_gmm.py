...
#         self.node_future_encoder_h = nn.Linear(6, 32)    # error input dim
        self.node_future_encoder_h = nn.Linear(4, 32)
...
#                                         torch.reshape(goal_loc_gmm.corrs, [-1, 1, 1]))
        ...
        #### only work for GMM4D
        flatten_goal_dist = self.GMM(torch.reshape(goal_loc_gmm.input_log_pis, [-1, 1]),
                                        torch.reshape(goal_loc_gmm.mus, [-1, 1, 4]),
                                        torch.reshape(goal_loc_gmm.log_sigmas, [-1, 1, 4]),
                                        torch.reshape(goal_loc_gmm.corrs, [-1, 1, 2]))
