...
from bitrap.utils.box_utils import cxcywh_to_x1y1x2y2, x1y1x2y2_to_cxcywh
import torch
...
def evaluate_unimodal(pred_traj, target_traj, mode='bbox', distribution=None, bbox_type='x1y1x2y2'):
    '''
    pred_traj: (B, T, 4)
    target_traj: (B, T, 4)
    '''    
    if mode == 'bbox':
        eval_results = evaluate_bbox_traj_unimodal(pred_traj, target_traj, bbox_type=bbox_type)
    elif mode == 'point':
        eval_results = {}
        ...
        eval_results['FDE'] = np.min(traj_FDE, axis=1).mean()
        # eval_results = evaluate_point_traj(pred_traj[range(len(best_idx)), best_idx], target_traj)
        eval_results['per_step_displacement_error'] = np.linalg.norm(pred_traj - target_traj, axis=-1).min(axis=2).mean(axis=0)
    else:
        raise NameError('Wrong mode')
    return eval_results
...
######## used for multimodal prediction
...
######## used for unimodal prediction
def evaluate_bbox_traj_unimodal(pred_traj, target_traj, bbox_type='x1y1x2y2'):
    '''
    pred_traj: (B, T, 4)
    target_traj: (B, T, 4)
    boxes are in x1y1x2y2 format
    ...
    # NOTE: mean over dim and time, then min over K samples, then mean over batch
    eval_results['ADE(0.5s)'] = per_step_box_mse[:, 0:15].mean()
    eval_results['ADE(1.0s)'] = per_step_box_mse[:, 0:30].mean()
    eval_results['ADE(1.5s)'] = per_step_box_mse.mean()
    eval_results['FDE'] = per_step_box_mse[:, -1].mean()
    ...
    eval_results['C-ADE(0.5s)'] = per_step_center_mse[:, 0:15].mean()
    eval_results['C-ADE(1.0s)'] = per_step_center_mse[:, 0:30].mean()
    eval_results['C-ADE(1.5s)'] = per_step_center_mse.mean()
    eval_results['C-FDE'] = per_step_center_mse[:, -1].mean()
    return eval_results
...
#     '''
#     
#     kde_list_forGoal = []    # for each trajectory
#     
#     kde_ll = 0.
#     
...
#                 pdf = np.clip(kde.logpdf(target_traj[batch_num, timestep].T), a_min=log_pdf_lower_bound, a_max=None)[0]     # is it a scalar? yes
#                 kde_ll += pdf / (T * batch_size)
#                 kde_ll_per_step[timestep] += pdf / batch_size
#                 
#                 #### my adding
#                 if timestep==(T-1):
#                     kde_list_forGoal.append(kde)
#                 
...
#                 #### my adding
#                 if timestep==(T-1):
#                     kde_list_forGoal.append(None)
#                 
#     return -kde_ll, -kde_ll_per_step, kde_list_forGoal
...
def compute_goal_kde(pred_traj):
    '''
    pred_traj: (batch, T, K, 2/4)
    '''
    kde_list_forGoal = []    # for each trajectory
