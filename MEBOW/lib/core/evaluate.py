...
# my adding
def draw_pred_orientation(img_np, pred_ori, path, img_name):
    for idx in range(len(pred_ori)):
        ...
        ax.set_ylim(-1.2, 1.2)
        ax.axis('off')
        fig.savefig(os.path.join(path, img_name+'.jpg'))
        plt.close()
        ori_img = cv2.imread(os.path.join(path, img_name+'.jpg'))

        width = img_tmp.shape[1]
        ori_img = cv2.resize(ori_img, (width, width), interpolation=cv2.INTER_CUBIC)
        img_all = np.concatenate([img_tmp, ori_img],axis=0)
        im = Image.fromarray(img_all)
        im.save(os.path.join(path, img_name+'_raw.jpg'))
